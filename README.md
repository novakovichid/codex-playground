...
1. Соберите и запустите сервисы: `docker-compose up --build`.
2. API будет доступен на `http://localhost:8000`, фронтенд — на `http://localhost:5173`.
3. При изменениях в коде контейнеры обновятся автоматически благодаря volume-монтажам.
